...
- 📡 **Modo Auto-Sintonización Dinámica en Vivo (`[AA]`):** Escucha todos los canales conocidos y, si la señalización anuncia una nueva portadora dentro del ancho de banda ($2.048\text{ MHz}$), **la sintoniza automáticamente en caliente en tiempo real**.
- 🗂️ **Agrupador Inteligente de Frecuencias (*SDR Bandwidth Clusters*):** Agrupa automáticamente todas las frecuencias descubiertas fuera de banda en **bloques contiguos de $\le 1.8\text{ MHz}$** (Grupos `[G1]` a `[G6]`) sin duplicados, permitiendo saltar a escuchar cualquier grupo completo de la red desde el menú interactivo.
- 📍 **Decodificación de Telemetría GPS / AVL:** Extracción en tiempo real de coordenadas geográficas (Latitud / Longitud en formatos NMEA y binario LIP) transmitidas por terminales y vehículos, con registro histórico en log.
- 🔑 **Detección de Eventos OTAR y Seguridad:** Identificación en tiempo real de mensajes de autenticación (`D_AUTHENTICATION`), autorización y sesiones de renovación de claves (*Over-The-Air Rekeying*) en todos los modos de visualización.
- 🌐 **Panel Web y Mapa GPS en Vivo (`http://localhost:8080`):** Interfaz web moderna con mapa interactivo OpenStreetMap/Leaflet que posiciona emisores GPS en tiempo real, muestra el estado de las portadoras y permite escuchar las grabaciones de voz desde el navegador o el móvil.
